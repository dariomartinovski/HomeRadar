...
def extract_features(df_prepared):
    ...
    print(f"✓ Features shape: {X.shape}, Target shape: {y.shape} (Log-transformed)")
...
def create_model():
    ...
    params = {
        'objective': 'reg:squarederror', # Stay with this; it works great with log data
        'learning_rate': 0.05,           # Lowered from 0.1 to prevent overshooting
        'n_estimators': 350,             # Increased from 200 to give the model more time to learn
        'max_depth': 5,                  # Slightly shallower (5 instead of 6) to improve generalization
        'subsample': 0.8,
        'colsample_bytree': 0.7,         # Slightly reduced to force trees to be more diverse
        'min_child_weight': 5,           # Increased from 3 to prevent trees from focusing on tiny outliers
        'gamma': 0.1,                    # Added a small penalty for making new splits to keep the model "conservative"
        'random_state': 42,
        'n_jobs': -1
    }
...
def evaluate_model(model, X_train, X_test, y_train, y_test, feature_names):
    ...
    # Calculate metrics on REAL dollar amounts
...
#     PROPERTY_CATEGORY = 'FOR_SALE'
    ...
    for category in categories:
        try:
            ...
            # Save model
            model_filename = os.path.join(output_dir, f"{category.lower()}_property_price_model_{timestamp_str}.pkl")
            joblib.dump(model, model_filename)
            print(f"💾 Model: {model_filename}")

            # Save encoders
            encoders_filename = os.path.join(output_dir, f"{category.lower()}_label_encoders_{timestamp_str}.pkl")
            joblib.dump(label_encoders, encoders_filename)
            print(f"💾 Encoders: {encoders_filename}")

            # Save features
            features_filename = os.path.join(output_dir, f"{category.lower()}_features_{timestamp_str}.json")
