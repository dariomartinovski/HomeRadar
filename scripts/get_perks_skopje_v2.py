...
data = response.json()
print(f"Received {len(data['elements'])} elements from Overpass API.")

results = []
type_counts = {}
fallback_counters = {}

for idx, element in enumerate(data["elements"], start=1):
    ...
    if lat and lon and perk_type:
        if not name:
            fallback_counters.setdefault(perk_type, 0)
            fallback_counters[perk_type] += 1
            name = f"UNKOWN_{perk_type}_{fallback_counters[perk_type]}"
        ...
        type_counts[perk_type] = type_counts.get(perk_type, 0) + 1
    else:
        print(f"[Skipped] idx={idx} name={name} type={perk_type} opening_hours={opening_hours} lat={lat} lon={lon}")

# Save to JSON
with open("skopje_perks.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2, ensure_ascii=False)

print(f"\n✅ Saved {len(results)} valid places to 'skopje_perks.json'.\n")
print("📊 Type summary:")
for perk, count in type_counts.items():
    print(f"  - {perk}: {count}")
